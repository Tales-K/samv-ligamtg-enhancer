 ...
 *   5. Re-run automatically when the SPA re-renders the card list.
 *
...
function updateGroupTotals(priceMap) {
  ...
  groupUls.forEach((ul) => {
    ...
    // Keep the en-dash separator that Moxfield uses (–&nbsp;$X.XX) and append BRL total.
    ...
    priceSpan.textContent = `${originalGroupText}  ·  ${fmtBRL(total)}`;
  });
